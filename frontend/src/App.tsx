...
} from './utils/appScenarios';
import { useAuthStore } from './store/auth';
import { useDefaultsStore, type DefaultsData } from './store/defaults';
...
const normaliseInputs = (
    parsed: Partial<Inputs> | null | undefined,
): Inputs => ({
    ...EMPTY_INPUTS,
    ...(parsed ?? {}),
});

const buildDefaultInputPresets = (
    p10Defaults: DefaultsData | null,
    p12Defaults: DefaultsData | null,
) =>
    ({
        p10: normaliseInputs((p10Defaults ?? {}) as Partial<Inputs>),
        p12: normaliseInputs((p12Defaults ?? {}) as Partial<Inputs>),
    }) as const;

type DefaultPresetKey = 'p10' | 'p12';

// SEEDED_DEFAULT_INPUTS will be computed dynamically based on loaded defaults
...
const computeQ2Results = (values: Record<FieldKey, number>): Q2Results => {
    ...
    return {
        ...
        totals: {
            ...totals,
            marginPct:
                totals.revenue > 0 ? totals.marginValue / totals.revenue : null,
        },
    };
};

const computeQ3ResultsFromQ2 = (q2Results: Q2Results): Q3Results => {
    const totalCost = q2Results.rows.reduce(
        (sum, row) => sum + row.totalCost,
        0,
    );
    const totalRevenue = q2Results.rows.reduce(
    ...
    const enrichedRows: Q3Row[] = q2Results.rows.map((row) => {
        ...
        const revenueSharePct =
            totalRevenue > 0 ? row.revenue / totalRevenue : null;
    ...
    const top3ByRatio = byRatio
        .filter((row) => row.costRevShareRatio !== null)
        .slice(0, 3);
...
function App() {
    ...
    const { defaults, loading: defaultsLoading } = useDefaultsStore();
...
function MainApp({ defaults: defaultsData }: MainAppProps) {
    ...
    const [activeDefaultPreset, setActiveDefaultPreset] =
        useState<DefaultPresetKey>('p12');
    ...
    const [scenarioSortOrder, setScenarioSortOrder] = useState<
        'original' | 'margin-desc'
    >('original');
    const [sliderConsol, setSliderConsol] = useState(0);
    const [sliderGroupage, setSliderGroupage] = useState(0);
    const [sliderStock, setSliderStock] = useState(0);
    const [baselineConsol, setBaselineConsol] = useState(0);
    const [baselineGroupage, setBaselineGroupage] = useState(0);
    const [baselineStock, setBaselineStock] = useState(0);
    const [lockedSliderType, setLockedSliderType] =
    ...
    const q2MissingQ1Fields = useMemo(() => {
        return Q2_REQUIRED_Q1_FIELDS.filter(
            (key) => parsedInputs[key] === null,
        );
    }, [parsedInputs]);
    ...
    const q2DerivedRowsPreview = useMemo(() => {
        ...
        return computeQ2DerivedRowsFromQ1(
            parsedInputs as Record<FieldKey, number>,
        );
    ...
    useEffect(() => {
        if (results) {
            try {
                const palletsOut = {
                    consol: parsedInputs.consol_pallet_out ?? 0,
                    groupage: parsedInputs.groupage_pallet_out ?? 0,
                    stock: parsedInputs.stock_pallet_out ?? 0,
                };
                const marginPerPallet = {
                    consol: results.perType.consol.marginPerPallet,
                    groupage: results.perType.groupage.marginPerPallet,
                    stock: results.perType.stock.marginPerPallet,
                };
                const scenarios = generateScenarioComparisons(
                    palletsOut,
                    marginPerPallet,
                );
                setScenarioComparisons(scenarios);
            } catch (err) {
    ...
    useEffect(() => {
        if (scenarioComparisons.length > 0) {
            ...
            setBaselineConsol(baseline.consolPct);
            setBaselineGroupage(baseline.groupagePct);
            setBaselineStock(baseline.stockPct);
            setSliderConsol(baseline.consolPct);
            setSliderGroupage(baseline.groupagePct);
            setSliderStock(baseline.stockPct);
        }
    }, [scenarioComparisons]);

    const handleSliderChange = (type: WorkTypeKey, newValue: number) => {
        const current = {
            consol: sliderConsol,
            groupage: sliderGroupage,
            stock: sliderStock,
        };
        const delta = newValue - current[type];
        ...
        newValues[type] = Math.max(0, Math.min(100, newValue));

        if (lockedSliderType && lockedSliderType !== type) {
            // One type is locked, apply delta inversely to the other non-locked type
            const unlockedTypes = (
                ['consol', 'groupage', 'stock'] as WorkTypeKey[]
            ).filter((t) => t !== type && t !== lockedSliderType);
            if (unlockedTypes.length > 0) {
                const unlockedType = unlockedTypes[0];
                const newUnlockedValue = newValues[unlockedType] - delta;
                // Clamp to prevent going below 0
                newValues[unlockedType] = Math.max(0, newUnlockedValue);
                // If clamped, adjust the current type to maintain 100%
                if (newUnlockedValue < 0) {
                    newValues[type] = 100 - (newValues[lockedSliderType] || 0);
                }
            }
        } else if (!lockedSliderType) {
            // No type locked, distribute delta across the other two
            const otherTypes = (
                ['consol', 'groupage', 'stock'] as WorkTypeKey[]
            ).filter((t) => t !== type);
            const deltaPerOther = delta / 2;
            const newOthers = otherTypes.map(
                (t) => newValues[t] - deltaPerOther,
            );

            // Check if any would go below 0
            const minOther = Math.min(...newOthers);
            if (minOther < 0) {
                // Clamp and redistribute
                for (const otherType of otherTypes) {
                    newValues[otherType] = Math.max(
                        0,
                        newValues[otherType] - deltaPerOther,
                    );
                }
            } else {
                for (const otherType of otherTypes) {
                    newValues[otherType] =
                        newOthers[otherTypes.indexOf(otherType)];
                }
            }
        }
        ...
        const currentTotal =
            Math.round(
                Object.values(newValues).reduce((a, b) => a + b, 0) * 100,
            ) / 100;
        if (Math.abs(currentTotal - 100) > 0.1) {
            const unlockedTypes = (
                ['consol', 'groupage', 'stock'] as WorkTypeKey[]
            ).filter((t) => t !== lockedSliderType);
            if (unlockedTypes.length > 0) {
                const adjustment = (100 - currentTotal) / unlockedTypes.length;
                for (const unlockedType of unlockedTypes) {
                    newValues[unlockedType] = Math.max(
                        0,
                        newValues[unlockedType] + adjustment,
                    );
                }
            }
        }

        setSliderConsol(Math.round(newValues.consol * 100) / 100);
        setSliderGroupage(Math.round(newValues.groupage * 100) / 100);
        setSliderStock(Math.round(newValues.stock * 100) / 100);
    };

    const handleSliderReset = () => {
        setSliderConsol(baselineConsol);
        setSliderGroupage(baselineGroupage);
        setSliderStock(baselineStock);
    };
    ...
    const handleQ2Calculate = () => {
        if (q2MissingQ1Fields.length > 0 || q2MissingQ2Fields.length > 0) {
            ...
            if (q2MissingQ1Fields.length > 0) {
                missingParts.push(
                    `Q1 required: ${q2MissingQ1Fields.join(', ')}`,
                );
            }
            if (q2MissingQ2Fields.length > 0) {
                missingParts.push(
                    `Q2 required: ${q2MissingQ2Fields.join(', ')}`,
                );
            }
        ...
        const computed = computeQ2Results(
            parsedInputs as Record<FieldKey, number>,
        );
    ...
    const handleLoadDefaults = (preset: DefaultPresetKey) => {
        setActiveDefaultPreset(preset);
        setInputs(DEFAULT_INPUT_PRESETS[preset]);
        clearCalculatedState();
    };

    const handleReset = () => {
        setInputs(DEFAULT_INPUT_PRESETS[activeDefaultPreset]);
        clearCalculatedState();
    };

    const handleClearSavedInputs = () => {
            ...
            {activeTab === 'q1' ? (
                <main className="content">
                    ...
                    <section className="actions">
                        <div>
                            ...
                            <button
                                type="button"
                                className="ghost-button"
                                onClick={() => handleLoadDefaults('p10')}
                            >
                                Load data for up to P10
                            </button>
                            <button
                                type="button"
                                className="ghost-button"
                                onClick={() => handleLoadDefaults('p12')}
                            >
                                Load data for up to P12
                            </button>
                            <button
                                type="button"
                                className="ghost-button"
                    ...
                    <section className="results">
                        ...
                        {results && scenarioComparisons.length > 0 && (
                            <div className="section-results scenarios-section">
                                <h4>Scenarios Comparison</h4>
                                <div className="scenarios-controls">
                                    <button
                                        className="sort-button"
                                        onClick={() =>
                                            setScenarioSortOrder(
                                                scenarioSortOrder === 'original'
                                                    ? 'margin-desc'
                                                    : 'original',
                                            )
                                        }
                                    ...
                                    <table>
                                        ...
                                        <tbody>
                                            ...
                                            'margin-desc'
                                                ...
                                                <tr key={idx}>
                                                    <td>{scenario.mix.name}</td>
                                                    <td>
                                                        {scenario.consolPct}%
                                                    </td>
                                                    <td>
                                                        {scenario.groupagePct}%
                                                    </td>
                                                    <td>
                                                        {scenario.stockPct}%
                                                    </td>
                        ...
                        {results && (
                            <div className="section-results slider-widget">
                                <h4>Distribution Optimizer</h4>
                                <div className="sliders-container">
                                    <div className="slider-group">
                                        <div className="slider-label-row">
                                            <label htmlFor="slider-consol">
                                                Consolidation:{' '}
                                                {Math.round(sliderConsol)}%
                                            </label>
                                            <button
                                                className={`lock-button ${lockedSliderType === 'consol' ? 'locked' : ''}`}
                                                onClick={() =>
                                                    setLockedSliderType(
                                                        lockedSliderType ===
                                                            'consol'
                                    ...
                                    <div className="slider-group">
                                        <div className="slider-label-row">
                                            <label htmlFor="slider-groupage">
                                                Groupage:{' '}
                                                {Math.round(sliderGroupage)}%
                                            </label>
                                            <button
                                                className={`lock-button ${lockedSliderType === 'groupage' ? 'locked' : ''}`}
                                                onClick={() =>
                                                    setLockedSliderType(
                                                        lockedSliderType ===
                                                            'groupage'
                                    ...
                                    <div className="slider-group">
                                        <div className="slider-label-row">
                                            <label htmlFor="slider-stock">
                                                Stock: {Math.round(sliderStock)}
                                                %
                                            </label>
                                            <button
                                                className={`lock-button ${lockedSliderType === 'stock' ? 'locked' : ''}`}
                                                onClick={() =>
                                                    setLockedSliderType(
                                                        lockedSliderType ===
                                                            'stock'
                                ...
                                <div className="slider-summary">
                                    <div className="metric">
                                        <span>
                                            Estimated Distribution Margin
                                        </span>
                                        <strong>
                                            {(() => {
                                                const mpp = {
                                                    consol:
                                                        results.perType.consol
                                                            .marginPerPallet ??
                                                        0,
                                                    groupage:
                                                        results.perType.groupage
                                                            .marginPerPallet ??
                                                        0,
                                                    stock:
                                                        results.perType.stock
                                                            .marginPerPallet ??
                                                        0,
                                                };
                                                const palletsOut = {
                                                    consol:
                                                        parsedInputs.consol_pallet_out ??
                                                        0,
                                                    groupage:
                                                        parsedInputs.groupage_pallet_out ??
                                                        0,
                                                    stock:
                                                        parsedInputs.stock_pallet_out ??
                                                        0,
                                                };
                                                const totalPalletsOut =
                                                    palletsOut.consol +
                ...
                <main className="content">
                    <section className="question">
                        ...
                        <div className="question-method">
                            ...
                            <ul>
                                <li>
                                    Highest margin activity is ranked by margin
                                    %.
                                </li>
                                <li>
                                ...
                                <li>EXP and QC are entered directly for Q2.</li>
                            </ul>
                        </div>
                    </section>
                    ...
                    <section className="section-card">
                        <div className="section-header">
                            ...
                            <p className="muted">
                                ...
                                Confirm final DIS stream set before final
                                release.
                            </p>
                        </div>
                    </section>
                    ...
                    <section className="actions">
                        ...
                        <div className="status">
                            {q2Error && (
                                <span className="error">{q2Error}</span>
                            )}
                ...
                <main className="content">
                    <section className="question">
                        ...
                        <div className="question-method">
                            ...
                            <ul>
                                <li>
                                    Sort streams by total cost (high to low).
                                </li>
                                <li>
                                    Show revenue share and cost share by stream.
                                </li>
                                <li>
                        ...
                        <>
                            <section className="results">
                                <div className="section-header">
                                    <h3>Q3 summary</h3>
                                    <p className="muted">
                                        Portfolio cost/revenue concentration
                                        view by stream.
                                    </p>
                                </div>
                                <div className="totals-grid">
                            ...
                            <section className="section-results">
                                <h4>
                                    Top 3 cost-heavy / revenue-light streams
                                </h4>
                                <div className="section-metrics">
